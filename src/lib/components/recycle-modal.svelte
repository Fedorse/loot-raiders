...
{#if modalData && def}
	...
	>
		...
		>
			<div class="bg-modal px-4 py-3 text-modal-foreground md:px-5 md:py-4 lg:px-6 lg:py-5 2xl:px-7 2xl:py-6 3xl:px-8 3xl:py-7">
				<h1 class="mb-1.5 text-base leading-none font-black tracking-tight uppercase md:mb-2 md:text-lg lg:text-xl xl:text-2xl 2xl:mb-3 2xl:text-[28px] 3xl:text-[32px]">
					Recycle {def.name}
				</h1>

				<p class="mb-2.5 text-[10px] leading-snug font-medium text-modal-secondary-foreground md:mb-3 md:text-[11px] lg:text-xs xl:text-[13px] 2xl:mb-5 2xl:text-[15px] 3xl:text-[17px]">
					You have selected {modalData.item.count} item{modalData.item.count > 1 ? 's' : ''} to recycle.
					These are the resources you will get back:
				</p>

				<div class="flex min-h-[70px] flex-wrap gap-1.5 rounded bg-modal-secondary p-2 md:min-h-[80px] md:gap-2 md:p-2.5 lg:min-h-[90px] xl:min-h-[100px] 2xl:min-h-[110px] 2xl:gap-2.5 2xl:p-3 3xl:min-h-[130px] 3xl:gap-3 3xl:p-3.5">
					...
					{#each attachments as att (att.uid)}
						<ItemCard item={att} selected={false} className="h-14 w-14 md:h-16 md:w-16 lg:h-18 lg:w-18 xl:h-20 xl:w-20 2xl:h-[90px] 2xl:w-[90px] 3xl:h-[105px] 3xl:w-[105px]" readonly={true} />
					{/each}

					{#if resources.length === 0 && attachments.length === 0}
						<div class="flex w-full items-center justify-center text-[10px] font-bold text-black/30 md:text-[11px] lg:text-xs 2xl:text-sm 3xl:text-[15px]">
			...
			<div class="flex gap-2 bg-surface px-4 py-3 md:gap-2.5 md:px-5 md:py-4 lg:gap-3 lg:px-6 lg:py-5 2xl:gap-4 2xl:px-7 2xl:py-6 3xl:px-8 3xl:py-7">
				<button
